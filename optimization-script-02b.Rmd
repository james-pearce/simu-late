...
offers <- c(0, 0, 0, 0, # newsletter ?
            0, 0, 0, 0, # gift ?
            2, 108, 146, 64)*offer_scale %>%   # seminar
  round 
```
...
for (j in seq_along(offers)){
  if (offers[j] > 0) {
    offers_min[j] <- min(seq(n_alloc)[d_DT_alloc_0[[j + 4L]] >= offers[j]])
  }
}
offers_min %>% which.min
offers_min %>% min
```
 This means we are hitting the limit with the pension seminar at position 7996. 
...
repeat {
  ...
  for (j in seq_along(offers)){
    if (offers_rem[j] > 0) {
      offers_min[j] <- min(seq(n_alloc)[d_DT_alloc_0[[j + 4L]] >= offers_rem[j]])
    }
  }
...
## Calculate the baseline figures

Here I have the concept of a _baseline_. In the first instance, the baseline is a naive calculation: maximise row-wise profit.